...
export default async function Home(props: { searchParams: Promise<{ [key: string]: string | undefined }> }) {
  ...
  // since this MVP schema doesn't have an explicit 'category' database column yet.
  let whereClause = {};
  if (categoryFilter) {
    whereClause = {
      OR: [
      ...
      <div className="container mx-auto px-4 md:px-6 lg:px-8 py-10 max-w-7xl">
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-12 py-24 bg-[#F8FAFC] border border-slate-200/60 rounded-[24px] shadow-sm text-center mt-8">
            ...
            <p className="text-slate-500 max-w-md">Try clearing your filters or be the first to add one.</p>
          </div>
        ) : (
          <>
